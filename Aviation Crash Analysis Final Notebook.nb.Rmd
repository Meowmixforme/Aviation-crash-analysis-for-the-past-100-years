...
# Create a Weather column

```{r}



# Create a function to map tokens to weather values

get_weather <- function(tokens) {
  if ("fog" %in% tokens || "storm" %in% tokens || "weather" %in% tokens || "conditions" %in% tokens ||
      "poor" %in% tokens || "rain" %in% tokens || "icing" %in% tokens || "snow" %in% tokens ||
      "thunderstorm" %in% tokens || "ice" %in% tokens || "thunderstorms" %in% tokens ||
      "storm" %in% tokens || "lightning" %in% tokens || "snowstorm" %in% tokens ||
      "rainstorm" %in% tokens || "gust" %in% tokens || "overcast" %in% tokens || "winds" %in% tokens) {
    return("Bad Weather")
  } else {
...
aero$Weather <- sapply(aero$Summary, function(summary) {
...
get_cause_of_crash <- function(tokens) {
  if ("fog" %in% tokens || "storm" %in% tokens || "weather" %in% tokens ||
    ...
    return("Bad Weather")
             ...
             "operation" %in% tokens || "captain's" %in% tokens ||
             "planning" %in% tokens || "" %in% tokens) {
    return("Human Error")
  } else if ("war" %in% tokens || "terrorism" %in% tokens) {
    return("Act of War / Terrorism")
  } else if ("mechanical" %in% tokens || "failure" %in% tokens) {
    return("Mechanical Failure")
  } else if ("natural" %in% tokens || "disaster" %in% tokens) {
