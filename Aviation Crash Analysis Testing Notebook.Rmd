...
```{r}
## Time (I may just drop the column as 42.1% of values are missing)
...
# Create a frequency table of time values
time_freq <- table(aero$Times)

# Find the most common time value
most_common_time <- names(time_freq)[which.max(time_freq)]
...
library(missForest)
imputed_single_column <- missForest(data.frame(aero))
...
imputed_data <- mice(aero, method = "pmm", m = 5)  # You can adjust 'm' as needed

# Retrieve the imputed data (complete datasets)
aero <- complete(imputed_data)
